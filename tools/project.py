        ...
        def add_unit(build_obj: BuildConfigUnit, link_step: LinkStep):
            ...
            if obj is None:
                ...
                if obj_path is not None:
                    if config.platform == Platform.PS2:
                        # Unconfigured PS2 unit: assemble the splat asm stub so the
                        # .o exists for objdiff and the linker.
                        # Guard: skip if obj_name has no valid filename component
                        # (can happen with c-type segments whose name is ".")
                        if not Path(obj_name).name:
                            link_step.add(Path(obj_path))
                            return
                        ps2_asm_path = (
                            build_path
                            / "asm"
                            / Path(obj_name).with_suffix(
                                Path(obj_name).suffix + ".s"
                            )
                        )
                        ps2_obj = Path(obj_path)
                        if ps2_obj not in source_added and ps2_asm_path.exists():
                            source_added.add(ps2_obj)
                            asflags_str = make_flags_str(config.asflags)
                            n.comment(f"{obj_name}: (unconfigured)")
                            n.build(
                                outputs=ps2_obj,
                                rule="as",
                                inputs=ps2_asm_path,
            ...
            if obj.src_path is not None and obj.src_path.exists():
                ...
                # PS2: assemble the splat asm stub as the reference object when
                # no C++ source has been written yet.
                if config.platform == Platform.PS2:
                    asm_path = Path(
                        str(
                            obj.src_obj_path.with_suffix(
                                Path(obj_name).suffix + ".s"
                            )
                        ).replace("src", "asm")
                    )
                    ps2_obj_path = Path(
                        str(obj.src_obj_path).replace("src", "obj")
                    )
                    link_built_obj = True
                    built_obj_path = asm_build(obj, asm_path, ps2_obj_path)
...
def generate_objdiff_config(
    ...
    def add_unit(
        ...
        obj = objects.get(obj_name)
        if obj is None:
            if config.platform == Platform.PS2 and Path(obj_name).name:
                # For unconfigured PS2 units: only keep target_path if the
                # asm stub exists (asmtu type). c-type units have no single
                # asm stub and no build rule yet, so null out target_path
                # to avoid objdiff crashing on a missing file.
                ps2_asm_path = (
                    config.out_path()
                    / "asm"
                    / Path(obj_name).with_suffix(Path(obj_name).suffix + ".s")
                )
                if not ps2_asm_path.exists():
                    unit_config["target_path"] = None
